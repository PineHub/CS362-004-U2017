...
int main() {
	...
	// ----------- TEST 1: council room played ------
	printf("TEST 1: council room played\n");

	// copy the game state to a test case
	memcpy(&testG, &G, sizeof(struct gameState));

	council_room_card(&testG, thisPlayer, handpos);
	
	printf("new vs expected hand count:\n");
	totalAsserts += myAssert(testG.handCount[thisPlayer], G.handCount[thisPlayer]+3);
	printf("new vs expected deck count + discard count:\n");
	totalAsserts += myAssert(testG.deckCount[thisPlayer] + testG.discardCount[thisPlayer], G.deckCount[thisPlayer] + G.discardCount[thisPlayer] -4);
	printf("new vs expected hand count player 2: \n");
	totalAsserts += myAssert(testG.handCount[thisPlayer+1], G.handCount[thisPlayer+1]+1);
	printf("new vs expected deck count + discard count player 2:\n");
	totalAsserts += myAssert(testG.deckCount[thisPlayer+1] + testG.discardCount[thisPlayer+1], G.deckCount[thisPlayer+1]+G.discardCount[thisPlayer+1]-1);
	...
	printf("new vs expected buys:\n");
	totalAsserts += myAssert(testG.numBuys, G.numBuys + 1);
