...
int main() {
	...
	printf("TEST 1: village played\n");

	// copy the game state to a test case
	memcpy(&testG, &G, sizeof(struct gameState));

	villageCard(thisPlayer, &testG, handpos);

	printf("new vs expected hand count:\n");
	totalAsserts += myAssert(testG.handCount[thisPlayer], G.handCount[thisPlayer]);
	printf("new vs expected deck count + discard count:\n");
	totalAsserts += myAssert(testG.deckCount[thisPlayer] + testG.discardCount[thisPlayer], G.deckCount[thisPlayer] + G.discardCount[thisPlayer] -1);
	...
	printf("new vs expected actions:\n");
	totalAsserts += myAssert(testG.numActions, G.numActions + 2);
